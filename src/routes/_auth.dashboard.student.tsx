...
function StudentDash() {
  const classes = [
    { day: "Mon", time: "5:00 PM", subject: "Maths", topic: "Algebra · Linear equations", status: "upcoming" as const },
    { day: "Wed", time: "5:00 PM", subject: "English", topic: "Speaking Lab · Storytelling", status: "upcoming" as const },
    { day: "Fri", time: "5:00 PM", subject: "Science", topic: "Light · Reflection", status: "completed" as const },
  ];
  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <StatCard label="AI questions solved" value={148} sub="This week" trend="up" icon={Sparkles} />
            ...
            <Panel key={i} className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-4 min-w-0">
                <div className="h-12 w-12 rounded-xl bg-accent/30 flex flex-col items-center justify-center shrink-0">
                  <span className="text-[10px] uppercase tracking-wider font-semibold">{c.day}</span>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {practice.map((p) => (
            <Panel key={p.title} className="hover:border-primary transition flex flex-col">
              ...
              <div className="mt-1 text-xs text-muted-foreground flex-1">{p.count} · {p.time}</div>
