...
interface Post { id: string; slug: string; title: string; excerpt: string | null; cover_emoji: string | null; author: string | null; created_at: string; }
  ...
  return (
    <div className="min-h-screen bg-background px-6 py-12">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mt-3 max-w-xl text-muted-foreground">Articles on confidence, learning, and parenting confident kids.</p>
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {posts.map(p => (
            <Link key={p.id} to="/blog/$slug" params={{ slug: p.slug }}
              ...
              <div className="mt-4 text-xs uppercase tracking-widest text-accent-foreground/70">{p.author}</div>
            </Link>
          ))}
        </div>
      </div>
