...
function BookDemo() {
  const [form, setForm] = useState({
    parent_name: "", child_name: "", child_grade: "", email: "", phone: "", preferred_time: "", notes: "",
  });
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    else { setDone(true); toast.success("Booking received! We'll be in touch soon."); }
  };
  ...
  return (
    <div className="min-h-screen bg-background px-6 py-12">
      <div className="mx-auto max-w-2xl">
        ...
        <p className="mt-3 text-muted-foreground">A 30-minute session. We'll assess and advise — no obligation.</p>
        {done ? (
          <div className="mt-10 card-ink rounded-3xl bg-accent/20 p-8 border border-accent">
            ...
            <p className="mt-3 text-sm">Our team will email you within 24 hours to confirm your slot.</p>
          </div>
        ) : (
          <form onSubmit={submit} className="mt-10 space-y-4 card-ink rounded-3xl bg-card p-8 border border-ink/15">
            {[
              ...
              <div key={f.k}>
                ...
                <input type={f.type ?? "text"} value={(form as never)[f.k]}
            ...
            <div>
              ...
              <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })}
                rows={4} className="mt-1 w-full rounded-xl border border-ink/20 bg-background px-4 py-3 text-sm" />
            </div>
